...
def rpg_create_confusion_matrix(narrative, classification):
    expected  = [RPG_EMOTIONS.index(line[1]) for line in narrative.values()]
...
def rpg_print_confusion_matrix(conf_matrix):
    ...
    line += "".join([ " " for i in range(longest_emotion_length + 1)])
    line += "".join([ emotion + " " for emotion in RPG_EMOTIONS])
    print(line)
    ...
    for row in conf_matrix:
        ...
        line += "".join([ " " for i in range(diff_lenght + 1)])
        ...
        for column in row:
            ...
            line += "".join([ " " for i in range(diff_lenght)])
            index_j += 1
